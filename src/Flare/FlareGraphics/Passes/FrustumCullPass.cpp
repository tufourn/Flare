...
FrustumPlanes FrustumCullPass::getFrustumPlanes(glm::mat4 mat, bool normalize) {
  ...
  if (normalize) { // hessian normal form
    planes.left /=
        glm::length(glm::vec3(planes.left.x, planes.left.y, planes.left.z));
    planes.right /=
        glm::length(glm::vec3(planes.right.x, planes.right.y, planes.right.z));
    planes.bottom /= glm::length(
        glm::vec3(planes.bottom.x, planes.bottom.y, planes.bottom.z));
    planes.top /=
        glm::length(glm::vec3(planes.top.x, planes.top.y, planes.top.z));
    planes.near /=
        glm::length(glm::vec3(planes.near.x, planes.near.y, planes.near.z));
    planes.far /=
        glm::length(glm::vec3(planes.far.x, planes.far.y, planes.far.z));
  }
